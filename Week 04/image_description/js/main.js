...
function featureLinkHandler(evt) { //evt parameter is an object representing the event as it is passed to our method
    ...
    if(featureImage.classList.contains('hidden')) {
        ...
        featureLink.innerHTML = featureLink.innerHTML.replace('View', 'Hide');
    } else {
        // Hide the image
        featureImage.src = ''; // to remove the image from the DOM
        featureImage.classList.add('hidden');
        featureLink.innerHTML = featureLink.innerHTML.replace('Hide', 'View');
    }
